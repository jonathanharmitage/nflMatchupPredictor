...
def display_options(options):
    ...
    while True:
        print("")
        mapper = {
            str(index + 1): options[key] for index, key in enumerate(options.keys())
        }
...
def view_teams():
    options = {
        "Teams-to-Abbrev": view_teams_to_abbrev,
...
def view_general_data_scraping():
    options = {"View Schedule": view_general_schedule, "View Teams": view_general_teams}
...
def view_data_scraping():
    options = {
        "Team Data Scraping": view_team_data_scraping,
...
def main():
    options = {"View Teams": view_teams, "Data Scraping Functions": view_data_scraping}
    os.system("clear")
